...
public class DeleteFromModelCommandHandler extends
    ...
    protected Command getCommand() {
        ...
        while (it.hasNext()) {
            ...
            if (!(editPart instanceof DiagramEditPart)) {
                final Command c = getCommand(editPart);
    ...
    private Command getCommand(final IGraphicalEditPart editPart) {
        final EditPart theParentEditPart = editPart.getParent();
        return !(theParentEditPart instanceof IGraphicalEditPart)
                || editPart.resolveSemanticElement().eContainer() == ((IGraphicalEditPart) theParentEditPart)
                        .resolveSemanticElement() ? editPart.getCommand(new EditCommandRequestWrapper(
                new DestroyElementRequest(false))) : editPart.getCommand(new GroupRequest(RequestConstants.REQ_DELETE));
    }
